...
- (void)loadImageForScrollViewWithFrame:(CGRect)frame{
    ...
    if (self.imageArr.count == 1) {//防止网上请求到的图片数据只有一个而因索引不存在崩溃
        ...
        [_middleImg sd_setImageWithURL:[NSURL URLWithString:self.imageArr[0]]];
        self.scrollEnabled = NO;
    }
    if (self.imageArr.count>1) {
        [_leftImg sd_setImageWithURL:[NSURL URLWithString:self.imageArr[self.imageArr.count-1]]];
        [_middleImg sd_setImageWithURL:[NSURL URLWithString:self.imageArr[0]]];
        [_rightImg sd_setImageWithURL:[NSURL URLWithString:self.imageArr[1]]];
    }
}
//时刻监听方向的改变,给方向属性赋值
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context{
    ...
    [_leftImg sd_setImageWithURL:[NSURL URLWithString:self.imageArr[leftIndex]]];
    [_rightImg sd_setImageWithURL:[NSURL URLWithString:self.imageArr[rightIndex]]];
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    [_middleImg sd_setImageWithURL:[NSURL URLWithString:self.imageArr[_currentIndex]]];
