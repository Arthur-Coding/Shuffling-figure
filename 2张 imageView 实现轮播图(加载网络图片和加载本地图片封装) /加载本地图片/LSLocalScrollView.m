...
- (void)loadImageForScrollViewWithFrame:(CGRect)frame{
    ...
    if (self.imageArr.count == 1) {//防止图片数据只有一个而因索引不存在崩溃
        ...
        _currentImg.image = [UIImage imageNamed:self.imageArr[0]];
        self.scrollEnabled = NO;
    }
    if (self.imageArr.count>1) {
        _currentImg.image = [UIImage imageNamed:self.imageArr[0]];
        _afterImg.image = [UIImage imageNamed:self.imageArr[1]];
    }
}
//时刻监听方向的改变,给方向属性赋值
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context{
    ...
    _afterImg.image = [UIImage imageNamed:self.imageArr[_afterIndex]];
}
